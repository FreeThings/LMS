    ...
    public class CommonController : Controller
    {
            ...
            var query = from d in db.Departments
                select new
                {
                    ...
                    courses = (from co in db.Courses
                        where co.Departments == d.Abbreviation
                        select new
                        {
                            number = co.Number,
                            cname = co.Name
        ...
        public IActionResult GetClassOfferings(string subject, int number)
        {
            var query = from c in db.Classes
                join p in db.Professors on c.Professor equals p.UserId
                join co in db.Courses on c.Course equals co.CourseId
                where co.Number == number && co.Departments == subject
                select new
                {
                    season = c.Season,
                    year = c.Year,
        ...
        public IActionResult GetAssignmentContents(string subject, int num, string season, int year, string category, string asgname)
        {
            var query = from a in db.Assignments
                ...
                where co.Number == num && d.Abbreviation == subject &&
        ...
        public IActionResult GetSubmissionText(string subject, int num, string season, int year, string category, string asgname, string uid)
        {
            var query = from s in db.Submissions
                ...
                select new
                {
                    submissionText = s.Contents
                };
            var result = query.FirstOrDefault();
            return Content(result?.submissionText ?? "");
        }
        ...
        public IActionResult GetUser(string uid)
        {
            ...
            if (studentQuery.Any())
            {
                var student = from s in db.Students
                    join d in db.Departments on s.Major equals d.Abbreviation
                    where s.UserId == uid
                    select new
                    {
                        fname = s.FirstName,
            ...
            if (professorQuery.Any())
            {
                var professor = from p in db.Professors
                    join d in db.Departments on p.Department equals d.Abbreviation
                    where p.UserId == uid
                    select new
                    {
                        fname = p.FirstName,
